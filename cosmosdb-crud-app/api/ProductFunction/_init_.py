...
def create_product(req):
    ...
    if "id" not in product or "Category" not in product:
        return func.HttpResponse(json.dumps({"error": "ID and Category are required!"}),
                                 mimetype="application/json", status_code=400)
    try:
        container.create_item(body=product)
        return func.HttpResponse(json.dumps({"message": f"Product {product['id']} created successfully!"}),
                                 mimetype="application/json", status_code=201)
    except exceptions.CosmosResourceExistsError:
        return func.HttpResponse(json.dumps({"error": f"Product {product['id']} already exists!"}),
...
def read_product(req):
    ...
    try:
        ...
        return func.HttpResponse(json.dumps({"error": "Product not found"}),
...
def update_product(req):
    ...
    return func.HttpResponse(json.dumps({"message": f"Product {updated_product['id']} updated successfully!"}),
...
def delete_product(req):
    ...
    try:
        ...
        return func.HttpResponse(json.dumps({"message": f"Product {product_id} deleted successfully!"}),
                                 mimetype="application/json")
    except exceptions.CosmosResourceNotFoundError:
        return func.HttpResponse(json.dumps({"error": "Product not found"}),
                                 mimetype="application/json", status_code=404)
